...
| ブランチ | 役割 | 説明 |
|---------|------|------|
| `main` | 本番 | リリース済みの安定コードのみ。直接コミット禁止。 |
| `develop` | 開発 | 日常の開発はすべてここで行う。機能完成後に main へマージ。 |

### フロー
```
develop（開発） ──→ main（本番リリース時にマージ）
```

- 新機能・バグ修正はすべて `develop` ブランチで作業する
- `main` へのマージはリリース時のみ。必ず動作確認済みの状態で行う
- `git push origin develop` で開発成果を共有する
